...
const Main = () => {
  ...
  const refRetailMap = useMemo(() => {
    ...
    allProducts.forEach((p: any) => {
      ...
      const halfBoxUnit = units.find((u: any) => u.pricingRule === 'half-box' && u.boxRetailPrice > 0)
      if (halfBoxUnit) {
      ...
      const retailPrices = units.map((u: any) => u.retailPrice ?? 0).filter((v: number) => v > 0)
  ...
  const totalProfit = useMemo(
    () => allSold.reduce((sum: number, p: any) => sum + calcProfit(p.revenue ?? 0, p.productId), 0),
  ...
  return (
    <main className="w-full flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <StatCard
          label="Total Revenue"
          value={fmt(overview?.revenue?.total ?? 0)}
          sub={`Cash ${fmt(overview?.revenue?.cash ?? 0)} · MoMo ${fmt(overview?.revenue?.momo ?? 0)}`}
          icon={TrendingUp}
          color="bg-endeavour"
        />
        <StatCard
          ...
          sub={`Cash ${overview?.sales?.cash ?? 0} · MoMo ${overview?.sales?.momo ?? 0}`}
          icon={ShoppingCart}
          color="bg-green-500"
        />
        <StatCard
          ...
          sub={`${overview?.inventory?.totalStock ?? 0} units in stock`}
          icon={Package}
          color="bg-purple-500"
        />
        <StatCard
          label="Low Stock"
          value={overview?.inventory?.lowStockProducts ?? 0}
          sub="Products below minimum quantity"
          icon={AlertTriangle}
          color="bg-red-500"
        />
        <StatCard
          label="Avg Sale Value"
          value={fmt(overview?.revenue?.average ?? 0)}
          sub="Average revenue per transaction"
          icon={DollarSign}
          color="bg-amber-500"
        />
        <StatCard
          ...
          sub={totalProfit < 0 ? 'Negative — check purchasePrice matches selling unit' : 'Revenue minus purchase cost'}
          icon={TrendingUp}
