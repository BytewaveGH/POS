...
// ── Budget progress row ────────────────────────────────────────────────────────
const BudgetRow = ({
  label, actual, target, color, formatter = fmt,
}: {
  label: string; actual: number; target: number; color: string; formatter?: (v: number) => string
}) => {
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <p className="text-[10px] text-gray-400">
        {over
          ...
          : target > 0 ? `${formatter(target - actual)} remaining to target` : 'No target set'}
...
// ── Section header ─────────────────────────────────────────────────────────────
const SectionHeader = ({ icon: Icon, color, title, sub, action }: {
  icon: React.ElementType; color: string; title: string; sub?: string; action?: React.ReactNode
}) => (
...
const Main = () => {
  ...
  const applyChartPreset = (preset: 'this-week' | '7d' | '14d' | '30d') => {
    const now = new Date()
    if (preset === 'this-week') {
      const r = thisWeekRange(); setChartFrom(r.from); setChartTo(r.to)
    } else {
  ...
  const saveBudget = () => {
    ...
    setDailyTarget(d); setMonthlyTarget(m)
  ...
  // ── Data fetches ───────────────────────────────────────────────────────────
  const { data: overviewRaw,     refetch: refetchOverview }     = useFetchData(`analytics-ov-${dateKey}`,    StatisticsServices.FetchAll(dateParams)                                                    as unknown as IGeneric)
  const { data: prevOverviewRaw }                                = useFetchData(`analytics-ov-prev-${prevRange?.from}-${prevRange?.to}`, StatisticsServices.FetchAll({ from: prevRange?.from, to: prevRange?.to }) as unknown as IGeneric, !!prevRange)
  const { data: dailySalesRaw,   refetch: refetchDailyChart }   = useFetchData(`analytics-daily-${chartFrom}-${chartTo}`, StatisticsServices.FetchSales({ granularity: 'day', from: chartFrom, to: chartTo })  as unknown as IGeneric)
  const { data: allSoldRaw }                                     = useFetchData(`analytics-sold-${dateKey}`,  StatisticsServices.FetchProducts({ limit: 1000, ...dateParams })                           as unknown as IGeneric)
  const { data: chartSoldRaw }                                   = useFetchData(`analytics-csold-${chartFrom}-${chartTo}`, StatisticsServices.FetchProducts({ limit: 1000, from: chartFrom, to: chartTo }) as unknown as IGeneric)
  ...
  const monthlyRange = useMemo(() => {
    const now = new Date()
    const start = new Date(now); start.setMonth(start.getMonth() - 11); start.setDate(1)
    return { from: isoDate(start), to: isoDate(now) }
  }, [])
  const { data: monthlySalesRaw, refetch: refetchMonthlyChart } = useFetchData(`analytics-monthly-${monthlyRange.from}`, StatisticsServices.FetchSales({ granularity: 'month', from: monthlyRange.from, to: monthlyRange.to }) as unknown as IGeneric)

  const [topPage, setTopPage] = useState(0)
  const topPageSize = 8
  const { data: topProductsRaw, total: topTotal, refetch: refetchTop } = useFetchPaginated(`analytics-top-${dateKey}`, StatisticsServices.FetchProducts({ ...dateParams }) as unknown as IGeneric, topPage, topPageSize)
  ...
  const monthlySalesWithProfit = useMemo(
    () => monthlySalesArr.map((d: any) => ({
      ...d, profit: Number(((d.revenue ?? 0) * overallMarginRatio).toFixed(2)),
    })),
  ...
  const currentMonthRevenue = useMemo(() => monthlySalesArr.find((d: any) => (d.date as string)?.startsWith(currentMonthKey))?.revenue ?? 0, [monthlySalesArr, currentMonthKey])
  ...
  const insights = useMemo(() => {
    ...
    return list
  }, [totalRevenue, prevRevenue, totalSales, prevSales, profitMarginPct, topProducts, totalSoldRevenue, warehouses, momoRevenue, dailySalesWithProfit, shortages, lowStockCount, avgSaleValue, prevAvgSale])
  ...
  return (
    <main className="w-full flex flex-col gap-6">
      ...
      <header className="flex flex-col gap-3">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0 self-start">
            <button onClick={() => refetchOverview()} className="p-1.5 rounded-lg border border-gray-200 text-gray-400 hover:text-endeavour hover:border-endeavour transition-colors" title="Refresh all">
              <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
            </button>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-3 flex flex-col gap-2">
          ...
          <div className="flex rounded-lg border border-gray-200 overflow-hidden text-xs">
            {([
              ...
              <button key={opt.key} onClick={() => applyGlobalPreset(opt.key)}
      ...
      <section className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        <KpiCard
          label="Total Revenue" value={fmtShort(totalRevenue)}
          sub="vs prior period"
          icon={TrendingUp} color="bg-endeavour"
          trend={trendPct(totalRevenue, prevRevenue)}
        />
        <KpiCard
          label="Gross Profit" value={fmtShort(totalProfit)}
        ...
        />
        <KpiCard
          label="Avg Sale Value" value={fmtShort(avgSaleValue)}
          sub="per transaction"
          icon={BarChart2} color="bg-purple-500"
          trend={trendPct(avgSaleValue, prevAvgSale)}
        />
        <KpiCard
          label="Profit Margin" value={`${profitMarginPct.toFixed(1)}%`}
          sub="gross margin"
          icon={Percent}
          ...
          <div className="flex flex-col gap-2.5 mb-3">
            ...
            <div className="flex rounded-lg border border-gray-200 overflow-hidden text-xs">
              {([
                ...
                <button key={opt.key} onClick={() => applyChartPreset(opt.key)}
                  className="flex-1 py-1.5 bytewave-paragraph transition-colors whitespace-nowrap bg-white text-gray-500 hover:bg-gray-50 hover:text-endeavour border-l border-gray-200 first:border-l-0 text-center">
                  {opt.label}
                </button>
              ))}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 overflow-hidden flex flex-col">
          ...
          <div className="flex items-center justify-between px-4 py-2 border-t border-gray-100 text-xs text-gray-500">
            ...
            <div className="flex items-center gap-2">
              <button disabled={topPage === 0} onClick={() => setTopPage((p) => p - 1)}
              ...
              <button disabled={topPage >= Math.ceil(topTotal / topPageSize) - 1} onClick={() => setTopPage((p) => p + 1)}
                className="px-1.5 py-0.5 rounded border border-gray-200 disabled:opacity-30 hover:border-endeavour transition-colors">›</button>
            </div>
          </div>
        </div>
          ...
          {editingBudget ? (
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  type="number" value={draftDaily} onChange={(e) => setDraftDaily(e.target.value)}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  type="number" value={draftMonthly} onChange={(e) => setDraftMonthly(e.target.value)}
            ...
            <div className="flex flex-col gap-3">
              {(() => {
                ...
                return warehouses.slice(0, 7).map((w: any, i: number) => {
                  ...
                  const barColors = ['bg-purple-500', 'bg-endeavour', 'bg-emerald-500', 'bg-amber-500', 'bg-rose-500', 'bg-cyan-500', 'bg-indigo-500']
                  return (
              ...
              <div className="grid grid-cols-3 gap-2 pt-1">
                ...
                <div className="bg-gray-50 rounded-xl p-2.5 text-center">
                  ...
                  <p className="text-lg font-bold text-stone-800 mt-0.5">
                    {fmtShort(warehouses.length > 0 ? warehouses.reduce((s: number, w: any) => s + (w.revenue ?? 0), 0) / warehouses.length : 0)}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-xl p-2.5 text-center">
