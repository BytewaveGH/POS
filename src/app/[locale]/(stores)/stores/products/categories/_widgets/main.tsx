...
const Main = () => {
  ...
  } = useFetchData('units', UnitServices.FetchAll() as unknown as IGeneric)

  const {
    data: warehouses,
    isLoading: warehousesLoading,
    refetch: refetchWarehouses,
  } = useFetchData('warehouses', WarehouseServices.FetchAll() as unknown as IGeneric)
      ...
      { field: 'name', headerName: 'Unit Name' },
      {
        ...
        cellRenderer: ({ data }: any) => (
          <div className="flex items-center gap-1 h-full">
            ...
            <button className="text-xs text-red-500 hover:underline" onClick={() => handleDeleteUnit(data.id)}>
              Delete
            </button>
          </div>
        ),
      ...
      { field: 'address', headerName: 'Address', flex: 1 },
      {
        ...
        cellRenderer: ({ data }: any) => (
          <div className="flex items-center gap-1 h-full">
            ...
            <span className="text-gray-300">|</span>
            <button className="text-xs text-red-500 hover:underline" onClick={() => handleDeleteWarehouse(data.id)}>
              Delete
            </button>
          </div>
        ),
