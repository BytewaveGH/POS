    ...
    <div className="flex items-center justify-between px-3 py-2 border-t border-gray-200 bg-white text-xs text-gray-500">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1">
          ...
          >
            {[100, 300, 500, 1000].map(s => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
        <div className="flex items-center gap-1">
          ...
          <button disabled={page === 0} onClick={() => onPage(page - 1)}
            className="px-1.5 py-0.5 rounded border border-gray-200 disabled:opacity-30 hover:border-endeavour transition-colors">‹</button>
          <span className="px-2 font-medium text-stone-600">Page {page + 1} of {totalPages}</span>
...
const Main = () => {
  ...
  const { data: operations, total: opTotal, isLoading: opLoading, refetch: refetchOperations } = useFetchPaginated(
    'operations',
    OperationsServices.FetchAll() as unknown as IGeneric,
    opPage,
    opPageSize,
  )
  ...
  const [restockTarget, setRestockTarget] = useState<{
    ...
    productName: string; warehouseName: string; currentQty: number
  } | null>(null)
  ...
  const { data: products, total: productTotal, isLoading, refetch } = useFetchPaginated(
    'products',
    ProductServices.FetchAll({}),
    productPage,
    productPageSize,
  )

  const { data: sales, total: salesTotal, isLoading: salesLoading, refetch: refetchSales } = useFetchPaginated(
    'sales',
    SalesServices.FetchAll() as unknown as IGeneric,
    salesPage,
    salesPageSize,
  )
  ...
  const { data: invoices, total: invoiceTotal, isLoading: invoicesLoading, refetch: refetchInvoices } = useFetchPaginated(
    'invoices',
    InvoiceServices.FetchAll() as unknown as IGeneric,
    invoicePage,
    invoicePageSize,
  )
  ...
  const { data: stocks, total: stockTotal, isLoading: stocksLoading } = useFetchPaginated(
    'stock-shortages',
    StocksServices.FetchShortages() as unknown as IGeneric,
    stockPage,
    stockPageSize,
  ...
  const handleRestock = async () => {
    ...
    try {
      ...
      const delta = restockMode === 'set'
        ? qty - restockTarget.currentQty
        : qty
  ...
  const shortageProducts = useMemo(() =>
    (products as any[]).filter((p: any) => p.isShortage)
  , [products])

  // Pending debt — invoices not yet paid (current page)
  const pendingInvoices = useMemo(() =>
    (invoices as any[]).filter((inv: any) => !inv.isPaid && inv.paymentStatus !== 'paid')
  , [invoices])

  const pendingDebt = useMemo(() =>
    pendingInvoices.reduce((sum: number, inv: any) => sum + (inv.totalAmount ?? 0), 0)
  , [pendingInvoices])
          ...
          <div className="flex items-center gap-2 h-full">
            ...
            {paid ? (
              <button
                ...
                onClick={() => handleMarkAsPending(row.id)}
              >
                Mark Pending
              </button>
            ) : (
    ...
    { field: 'notes',     headerName: 'Notes',      flex: 1 },
    {
      ...
      cellRenderer: ({ data: row }: any) => (
        <div className="flex items-center gap-2 h-full">
          <button
            className="text-xs text-endeavour hover:underline"
            onClick={() => { setSelectedOperation(row); setOperationModal(true) }}
          >Edit</button>
          <span className="text-gray-300">|</span>
          <button
            className="text-xs text-red-500 hover:underline"
  ...
  return (
    <div className='w-full h-full'>
      ...
      <SheetTemplate
        ...
        handleClose={() => { setOperationModal(false); setSelectedOperation(null) }}
        ...
        content={
          <CreateOperation
            ...
            onSuccess={() => { refetchOperations(); setOperationModal(false); setSelectedOperation(null) }}
          />
        }
      />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white rounded-2xl shadow-xl w-[540px] max-h-[85vh] flex flex-col overflow-hidden">
            ...
            <div className="px-5 py-4 border-b border-gray-100 flex items-start justify-between">
              ...
              <button onClick={() => setViewSale(null)} className="text-gray-400 hover:text-gray-600 text-lg leading-none">✕</button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {(viewSale.items ?? []).map((item: any, i: number) => (
                <div key={i} className="grid grid-cols-[1fr_4rem_6rem_6rem] gap-2 px-5 py-2.5 border-b border-gray-50 last:border-0 items-center">
                  <div>
            ...
            <div className="px-5 py-4 border-t border-gray-100 flex items-center justify-between">
              <span className="bytewave-paragraph text-sm text-gray-500">{(viewSale.items ?? []).length} item{(viewSale.items ?? []).length !== 1 ? 's' : ''}</span>
              <div className="flex items-center gap-3">
                <span className="bytewave-paragraph text-sm text-gray-500">Total</span>
                <span className="bytewave-heading text-xl text-stone-800">
                  GHC {Number(viewSale.totalAmount ?? 0).toLocaleString()}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white rounded-2xl shadow-xl w-[560px] max-h-[85vh] flex flex-col overflow-hidden">
            ...
            <div className="px-5 py-4 border-b border-gray-100 flex items-start justify-between">
              ...
              <button onClick={() => setViewInvoice(null)} className="text-gray-400 hover:text-gray-600 text-lg leading-none">✕</button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {(viewInvoice.items ?? []).map((item: any, i: number) => (
                <div key={i} className="grid grid-cols-[1fr_4rem_5rem_6rem_5rem] gap-2 px-5 py-2.5 border-b border-gray-50 last:border-0 items-center">
                  <div>
                    <p className="bytewave-paragraph text-sm text-stone-700">{item.productName}</p>
                    <p className="bytewave-paragraph text-xs text-gray-400">{item.unitName}</p>
                  </div>
                  <span className="bytewave-paragraph text-sm text-center text-stone-700">{item.quantity}</span>
                  <span className={`text-[10px] text-center font-medium px-1.5 py-0.5 rounded w-fit mx-auto ${
            ...
            <div className="px-5 py-4 border-t border-gray-100 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <span className="bytewave-paragraph text-sm text-gray-500">Total</span>
                <span className="bytewave-heading text-xl text-stone-800">
                  GHC {Number(viewInvoice.totalAmount ?? 0).toLocaleString()}
                </span>
              </div>
            </div>
          </div>
      ...
      {historyTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white rounded-2xl shadow-xl w-[520px] max-h-[80vh] flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto">
              {historyLoading ? (
                <div className="flex items-center justify-center h-32 bytewave-paragraph text-xs text-gray-400">
                  Loading history...
                </div>
                ...
                <>
                  ...
                  {historyRecords.map((h: any, i: number) => {
                    ...
                    const before =
                      h.previousQuantity ?? h.prevQuantity ?? h.oldQuantity ??
                      h.before ?? h.quantityBefore ?? h.beforeQuantity ??
                      h.old_quantity ?? h.prev_quantity ?? null

                    // Resolve "after" — quantity AFTER the change (new total)
                    const after =
                      h.newQuantity ?? h.currentQuantity ?? h.afterQuantity ??
                      h.after ?? h.quantityAfter ?? h.new_quantity ??
                      h.current_quantity ?? null

                    // Resolve "change" — the delta added/removed
                    const change =
                      h.change ?? h.delta ?? h.quantity ?? h.quantityChange ??
                      h.amount ?? h.restockQuantity ?? null
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="bytewave-paragraph text-right text-stone-600">
                          {displayBefore}
                        </span>
                        <span className={`bytewave-paragraph text-right font-semibold ${
                        ...
                        <span className="bytewave-paragraph text-right font-semibold text-stone-800">
                          {displayAfter}
                        </span>
