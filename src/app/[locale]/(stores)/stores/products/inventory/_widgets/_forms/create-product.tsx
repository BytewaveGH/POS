        ...
        <div key={fieldId} className="flex flex-col gap-2 p-3 rounded-lg border border-gray-200 bg-gray-50">
            ...
            {rule !== 'half-box' && (
                <div className="flex items-center gap-2">
                    <label className="bytewave-paragraph text-xs text-gray-500 whitespace-nowrap">
                        Wholesale from qty
                    </label>
                    <div className="w-24">
                        <InputsTemplate
                            control={control}
                            name={`units.${index}.bulkThreshold`}
                            placeholder="e.g. 5"
                            inputType="number"
                        />
                    </div>
                    <p className="bytewave-paragraph text-xs text-gray-400">
                        At this qty, price switches to wholesale
                    </p>
                </div>
            )}
...
const CreateProduct = ({ mode = 'create', productId, initialData, onSuccess }: CreateProductProps) => {
    ...
    const {
        ...
        append: appendStock,
        remove: removeStock,
    } = useFieldArray({ control: form.control, name: 'stock' })

    const handleAddPieceUnit = (
        detectedPieceUnitId: string,
        piecePrice: number,
        boxSizeNum: number,
    ...
    const resolveRule = (u: ProductForm['units'][number]) =>
        u.pricingRule !== 'flat' ? u.pricingRule
            : (u.bulkThreshold && u.bulkThreshold > 0 ? 'bulk-wholesale' as const : 'flat' as const)
    ...
    const onSubmit = async (data: ProductForm) => {
        ...
        try {
            if (mode === 'update' && productId) {
                ...
                await request(ProductServices.Update(productId, {
                    name:          data.name,
