...
export default function TenantDetail({ tenant }: TenantDetailProps) {
  ...
  const refetchEmployees = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/tenants/${tenant.id}/employees`)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await fetch(`/api/tenants/${tenant.id}/stats`)
        const body = await res.json()
  ...
  const toggleEmployeeStatus = async (employee: Employee) => {
    ...
    try {
      await fetch(`/api/tenants/${tenant.id}/employees/${employee.id}`, {
        method: 'PATCH',
